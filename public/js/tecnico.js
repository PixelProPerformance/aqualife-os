...
async function exportarPDF(dadosLaudo) {
  ...
  // NOTA DE SAÚDE
  const corUrg = cores[dadosLaudo.urgencia] || cores.ok;
  setFill([...corUrg, 0.1].slice(0,3).map(v => Math.min(255, v + 200 > 255 ? v : v + 160)));
  doc.roundedRect(M, y, CW, 22, 3, 3, 'F');
  doc.setFontSize(28); doc.setFont('helvetica','bold');
  setColor(corUrg);
  doc.text(String(dadosLaudo.nota || 0), M + 6, y + 15);
  doc.setFontSize(9); doc.setFont('helvetica','normal');
  setColor(cores.suave);
  doc.text('de 100', M + 22, y + 15);
  doc.setFontSize(13); doc.setFont('helvetica','bold');
  setColor(corUrg);
  const urgLabel = {ok:'Saudável',atencao:'Acompanhar',alerta:'Atenção',critico:'Crítico'}[dadosLaudo.urgencia]||'';
  doc.text(urgLabel, M + 50, y + 10);
  doc.setFontSize(9); doc.setFont('helvetica','normal');
  setColor(cores.suave);
  doc.text(`Confiança: ${dadosLaudo.confianca || 0}%`, M + 50, y + 17);
  y += 30;
