...
async function carregarEmail(){
  try{
    ...
    $("emailStatus").innerHTML=e.configurado
      ? `<span style="color:#065F46;font-weight:600">✓ Configurado</span> · ${e.smtp_host}:${e.smtp_port} (${e.smtp_user||""})`
      : `<span style="color:#92400E;font-weight:600">⚠ Ainda não configurado</span> — preencha para ativar os e-mails automáticos.`;
    ...
    if(e.smtp_from) $("smtpFrom").value=e.smtp_from;
...
$("salvarEmail").onclick=async()=>{
  ...
  try{
    await api("/api/admin/integracoes/email",{method:"PUT",body:JSON.stringify({
      ...
      smtp_pass:$("smtpPass").value, smtp_from:$("smtpFrom").value})});
    $("smtpPass").value="";
    carregarEmail();alert("Configuração SMTP salva.");
  }catch(e){alert("Erro: "+e.message);}finally{b.disabled=false;}
};
...
carregarClientes();carregarPessoas();carregarLaudos();carregarTickets();carregarSugestoes();carregarOferta();carregarMP();carregarEmail();
