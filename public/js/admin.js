...
async function carregarPedidos(){
  try{
    ...
    const bdg=s=>s==="pago"?'<span class="bdg bdg-pago">pago</span>':(s==="pendente"?'<span class="bdg bdg-pend">pendente</span>':'<span class="bdg bdg-canc">'+escH(s)+'</span>');
...
async function verPedido(id){
  ...
  try{
    ...
    $("pedidoCorpo").innerHTML=
      ...
      +'<p style="margin-top:12px;font-size:12.5px;color:var(--suave)">Status: <b>'+escH(p.status)+'</b> · pagamento: '+escH(p.status_pagamento||"—")+(p.mp_payment_id?" · MP "+escH(p.mp_payment_id):"")+'</p>';
  }catch(e){$("pedidoCorpo").innerHTML='<div class="vazio">Erro: '+e.message+'</div>';}
}
$("recarregarPedidos").onclick=carregarPedidos;

async function carregarLoja(){ await carregarCategorias(); carregarProdutos(); carregarPedidos(); }
