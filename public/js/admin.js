...
$("testarME").onclick=async()=>{
  ...
  out.style.color="var(--suave)"; out.textContent="Testando cálculo de frete…";
  try{
    const r=await api("/api/admin/integracoes/melhorenvio/teste",{method:"POST"});
    if(r.ok){ out.style.color="#065F46"; out.textContent="✓ Frete OK — "+r.n+" opção(ões) retornada(s). Está funcionando!"; }
    else{ out.style.color="#B4232E";
      out.innerHTML="✗ Falhou: "+escH(String(r.status||""))+(r.detalhe?(" — "+escH(typeof r.detalhe==="string"?r.detalhe:JSON.stringify(r.detalhe))):"")
        +"<br><span style='color:var(--suave)'>Dica: <b>me_http_401</b> = token inválido ou ambiente trocado. O token gerado em Produção só funciona com ambiente <b>Produção</b>; o de Sandbox só com <b>Sandbox</b>.</span>"; }
