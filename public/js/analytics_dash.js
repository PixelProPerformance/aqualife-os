...
async function carregarPaginas(){
  try{
    const d=await api("/api/admin/analytics/sections?range="+range);
    const sc=d.secoes||[];
    $("secoes").innerHTML=sc.length?`<table class="tbl"><thead><tr>
      <th>Seção</th><th class="num">Visitas</th><th class="num">Visitantes</th><th class="num">Tempo médio</th><th class="num">Scroll médio</th><th class="num">Cliques</th></tr></thead>
      <tbody>${sc.map(s=>`<tr>
        <td><b>${s.label}</b></td>
        <td class="num">${fmt(s.pageviews)}</td>
        <td class="num">${fmt(s.visitantes)}</td>
        <td class="num">${seg(s.tempo)}</td>
        <td class="num">${s.scroll}%</td>
        <td class="num">${fmt(s.cliques)}</td></tr>`).join("")}</tbody></table>`
      :'<div class="vazio">Sem dados no período.</div>';

    const bq=(d.aquabook&&d.aquabook.buscas)||[];
    $("abBuscas").innerHTML=bq.length?bq.map(b=>`<div class="bar-row" style="padding:5px 0"><span class="lbl">${b.q}</span><span class="num">${fmt(b.n)}</span></div>`).join("")
      :'<div class="vazio">Sem buscas registradas ainda. (O evento aquabook_search passa a alimentar isto automaticamente.)</div>';

    const sr=(d.aquabook&&d.aquabook.sem_resultado)||[];
    $("abSemResultado").innerHTML=sr.length?sr.map(b=>`<div class="bar-row" style="padding:5px 0"><span class="lbl">${b.q} <span class="tag0">0 resultados</span></span><span class="num">${fmt(b.n)}</span></div>`).join("")
      :'<div class="vazio">Nenhuma busca sem resultado. (Requer o site enviar props.resultados na busca.)</div>';

    const ac=d.academy||{};
    $("acVideos").innerHTML=(ac.videos_iniciados||ac.videos_concluidos)?`
      <div class="kpis" style="margin:0">
        <div class="kpi"><div class="rot">Vídeos iniciados</div><div class="val">${fmt(ac.videos_iniciados)}</div></div>
        <div class="kpi"><div class="rot">Vídeos concluídos</div><div class="val">${fmt(ac.videos_concluidos)}</div></div>
        <div class="kpi"><div class="rot">Taxa de conclusão</div><div class="val">${ac.taxa_conclusao}%</div></div>
      </div>`
      :'<div class="vazio">Sem vídeos assistidos ainda. (Vídeos &lt;video&gt; são captados automaticamente; players do YouTube entram numa fase futura.)</div>';
  }catch(e){ $("secoes").innerHTML=`<div class="vazio">Erro: ${e.message}</div>`; }
}

// ---------- ALERTAS ----------
async function carregarAlertas(){
  try{
    const d=await api("/api/admin/analytics/alerts?range="+range);
    const al=d.alertas||[];
    $("alertas").innerHTML=al.length?al.map(a=>`
      <div class="alert ${a.nivel}"><div class="ai">${a.icone||"⚠️"}</div>
        <div><div class="at">${a.titulo}</div><div class="ad">${a.detalhe}</div></div></div>`).join("")
      :'<div class="vazio">✅ Nenhum alerta no período. Tudo saudável por aqui.</div>';
  }catch(e){ $("alertas").innerHTML=`<div class="vazio">Erro: ${e.message}</div>`; }
}

// ---------- INSIGHTS IA ----------
function renderInsights(resumo,insights,quando){
  const cards=(insights||[]).map(i=>`
    <div class="ins"><span class="ins-cat">${i.categoria||"Insight"}</span>
      <div class="ins-tit">${i.titulo||""}</div>
      <div class="ins-rec">${i.recomendacao||""}</div>
      ${i.base?`<div class="ins-base">📊 ${i.base}</div>`:""}</div>`).join("");
  return `${resumo?`<div class="ins-resumo">${resumo}</div>`:""}
    ${quando?`<p style="font-size:12px;color:var(--suave);margin-bottom:12px">Gerado em ${new Date(quando).toLocaleString("pt-BR")}</p>`:""}
    ${cards||'<div class="vazio">Sem insights.</div>'}`;
}
async function carregarInsights(){
  try{
    const d=await api("/api/admin/analytics/insights");
    if(!d.ia){ $("insights").innerHTML='<div class="vazio">A IA não está configurada (falta a chave ANTHROPIC_API_KEY). Os Alertas continuam funcionando normalmente.</div>'; $("gerarInsights").disabled=true; return; }
    if(d.ultimo){ $("insights").innerHTML=renderInsights(d.ultimo.resumo,d.ultimo.insights,d.ultimo.gerado_em); }
    else{ $("insights").innerHTML='<div class="vazio">Nenhum relatório ainda. Clique em "Gerar insights" para a IA analisar os dados.</div>'; }
  }catch(e){ $("insights").innerHTML=`<div class="vazio">Erro: ${e.message}</div>`; }
}
$("gerarInsights").onclick=async()=>{
  const b=$("gerarInsights"); b.disabled=true; const t=b.textContent; b.textContent="Analisando…";
  $("insights").innerHTML='<div class="vazio">A IA está lendo os dados do período e montando as recomendações… (pode levar alguns segundos)</div>';
  try{
    const r=await fetch("/api/admin/analytics/insights",{method:"POST",headers:{authorization:"Bearer "+token,"content-type":"application/json"},body:JSON.stringify({range})});
    const d=await r.json();
    if(!r.ok) throw new Error(d.erro||"Falha ao gerar.");
    $("insights").innerHTML=renderInsights(d.resumo,d.insights,d.gerado_em);
  }catch(e){ $("insights").innerHTML=`<div class="vazio">Erro: ${e.message}</div>`; }
...
document.querySelectorAll(".aba2").forEach(a=>a.onclick=()=>{
  ...
  if(abaAtual==="paginas")carregarPaginas();
  if(abaAtual==="alertas")carregarAlertas();
  if(abaAtual==="insights")carregarInsights();
});
...
document.querySelectorAll("#rangesel button").forEach(b=>b.onclick=()=>{
  ...
  range=b.dataset.r;
  if(abaAtual==="funil")carregarFunil();
  else if(abaAtual==="paginas")carregarPaginas();
  else if(abaAtual==="alertas")carregarAlertas();
  else if(abaAtual==="insights")carregarInsights();
  else carregar();
});
