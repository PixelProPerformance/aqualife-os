...
// ============================================================
import { readFileSync as _rfs, existsSync as _exists } from "fs";
import { fileURLToPath as _ftu } from "url";

// Resolver caminho correcto dos dados independentemente do Root Directory
const _serverDir = path.dirname(_ftu(import.meta.url));
function _loadJson(nome) {
  const tentativas = [
    path.join(_serverDir, "../data", nome),         // src/../data (desenvolvimento)
    path.join(_serverDir, "data", nome),             // src/data
    path.join(process.cwd(), "src/data", nome),      // cwd/src/data
    path.join(process.cwd(), "data", nome),           // cwd/data
    path.join(process.cwd(), "aqualife-railway/src/data", nome), // Railway com subpasta
  ];
  for (const p of tentativas) {
    if (_exists(p)) {
      console.log(`[dados] ${nome} encontrado em: ${p}`);
      return JSON.parse(_rfs(p, "utf8"));
    }
  }
  console.warn(`[dados] AVISO: ${nome} não encontrado. Tentativas: ${tentativas.join(", ")}`);
  return [];
}

const _ESPECIES  = _loadJson("especies.json");
const _INCOMPATS = _loadJson("incompatibilidades.json");
