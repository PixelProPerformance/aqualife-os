...
app.post("/api/agendamento/checkout", async (req, res) => {
  ...
  try {
    ...
    res.json({ init_point: initPoint, valor_cobrado_cents: cobradoCents, recorrente: planoInfo.recorrente });
  } catch (err) {
    ...
    if (tipo === "payment") {
      ...
      await query(
        `INSERT INTO pagamento (subscription_id, user_id, mercadopago_payment_id, valor_cents,
                                moeda, status, metodo, raw_payload, external_reference)
         VALUES ($1,$2,$3,$4,'BRL',$5,$6,$7,$8)
         ON CONFLICT DO NOTHING`,
        [sub.id, sub.user_id, String(pg.id), Math.round((pg.transaction_amount || 0) * 100),
         pg.status, pg.payment_type_id || null, JSON.stringify(pg), extRef]
      );
