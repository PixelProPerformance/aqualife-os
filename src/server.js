...
app.delete("/api/admin/pessoas/:id", exigeLogin, exigeAdmin, async (req, res) => {
  ...
  try {
    const removido = await withTransaction(async (c) => {
      // 1) Quebra a cadeia pagamento → subscription (apaga os pagamentos do usuário
      //    antes das assinaturas, senão o FK impede a exclusão).
      ...
      //    (assim nada é esquecido, mesmo em tabelas futuras).
      ...
      for (const fk of fks.rows) {
        ...
        if (fk.notnull) {
          // Registro não faz sentido sem o usuário → remove (ex.: assinatura, ticket, progresso)
          await c.query(`DELETE FROM ${fk.tbl} WHERE ${fk.col} = $1`, [uid]);
        } else {
          // Histórico que preservamos, apenas desvinculando (ex.: laudos, aquários)
          await c.query(`UPDATE ${fk.tbl} SET ${fk.col} = NULL WHERE ${fk.col} = $1`, [uid]);
        }
      }

      // 3) Remove o usuário de fato — o e-mail volta a ficar livre.
