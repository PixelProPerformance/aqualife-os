...
// ============================================================
// ALERTAS INTELIGENTES — regras determinísticas sobre os eventos
// ============================================================
export async function alertas(range = "30d") {
  const janela = RANGES[range] || RANGES["30d"];
  const desde = `NOW() - INTERVAL '${janela}'`;
  const q1 = async (sql) => (await query(sql, [])).rows;
  const out = [];
  const add = (nivel, icone, titulo, detalhe) => out.push({ nivel, icone, titulo, detalhe });

  // 1) Páginas de saída rápida (tempo médio muito baixo)
  const rapidas = await q1(
    `SELECT path, ROUND(AVG((props->>'seconds')::numeric))::int s, COUNT(*)::int n
     FROM analytics_event WHERE event_name='time_on_page' AND created_at > ${desde} AND props ? 'seconds'
     GROUP BY path HAVING COUNT(*) >= 5 AND AVG((props->>'seconds')::numeric) < 12 ORDER BY n DESC LIMIT 5`);
  rapidas.forEach(p => add("atencao", "⏱️", "Saída rápida",
    `Em "${p.path}" o tempo médio é de só ${p.s}s (${p.n} visitas). Pode não estar prendendo a atenção.`));

  // 2) Páginas onde o usuário passa tempo demais (possível confusão)
  const longas = await q1(
    `SELECT path, ROUND(AVG((props->>'seconds')::numeric))::int s, COUNT(*)::int n
     FROM analytics_event WHERE event_name='time_on_page' AND created_at > ${desde} AND props ? 'seconds'
     GROUP BY path HAVING COUNT(*) >= 5 AND AVG((props->>'seconds')::numeric) > 240 ORDER BY s DESC LIMIT 3`);
  longas.forEach(p => add("info", "🧭", "Muito tempo na página",
    `Em "${p.path}" o tempo médio é de ${Math.round(p.s/60)} min (${p.n} visitas). Pode indicar conteúdo denso ou confuso.`));

  // 3) Páginas lentas (performance)
  const lentas = await q1(
    `SELECT path, ROUND(AVG((props->>'load_ms')::numeric))::int load, ROUND(AVG((props->>'lcp_ms')::numeric))::int lcp, COUNT(*)::int n
     FROM analytics_event WHERE event_name='perf' AND created_at > ${desde} AND props ? 'load_ms'
     GROUP BY path HAVING COUNT(*) >= 3 AND (AVG((props->>'load_ms')::numeric) > 4000 OR AVG((props->>'lcp_ms')::numeric) > 2500)
     ORDER BY load DESC LIMIT 5`);
  lentas.forEach(p => add("critico", "🐌", "Página lenta",
    `"${p.path}" carrega em ${(p.load/1000).toFixed(1)}s em média${p.lcp?` (LCP ${(p.lcp/1000).toFixed(1)}s)`:""}. Otimize para não perder visitantes.`));

  // 4) Buscas sem resultado no Aquabook
  const [semRes] = await q1(
    `SELECT COUNT(*)::int n, COUNT(DISTINCT props->>'q')::int termos
     FROM analytics_event WHERE event_name='aquabook_search' AND created_at > ${desde} AND (props->>'resultados')::int = 0`);
  if (semRes && semRes.n > 0) {
    const top = await q1(
      `SELECT props->>'q' q, COUNT(*)::int n FROM analytics_event
       WHERE event_name='aquabook_search' AND created_at > ${desde} AND (props->>'resultados')::int = 0
       GROUP BY props->>'q' ORDER BY n DESC LIMIT 3`);
    add("atencao", "🔎", "Buscas sem resultado",
      `${semRes.n} buscas no Aquabook não acharam nada (${semRes.termos} termos). Mais buscados: ${top.map(t => `"${t.q}" (${t.n}x)`).join(", ")}. Oportunidade de criar fichas.`);
  }

  // 5) Erros de JavaScript
  const [erros] = await q1(
    `SELECT COUNT(*)::int n, COUNT(DISTINCT props->>'message')::int tipos
     FROM analytics_event WHERE event_name='js_error' AND created_at > ${desde}`);
  if (erros && erros.n >= 5)
    add("critico", "🐞", "Erros de JavaScript",
      `${erros.n} erros de JS capturados (${erros.tipos} tipos). Podem estar quebrando funcionalidades para os visitantes.`);

  // 6) Maior abandono do funil
  try {
    const f = await funil(range);
    if (f.maior_abandono && f.maior_abandono.pct >= 70 && f.total >= 10)
      add("atencao", "🔻", "Abandono no funil",
        `${f.maior_abandono.pct}% dos usuários desistem entre "${f.maior_abandono.de}" e "${f.maior_abandono.para}". Priorize melhorar essa passagem.`);
  } catch (e) {}

  return { total: out.length, alertas: out };
}

// Monta um resumo compacto dos dados para a IA (Insights).
export async function contextoInsights(range = "30d") {
  const [vg, fn, sc, al] = await Promise.all([
    visaoGeral(range), funil(range), secoes(range), alertas(range),
  ]);
  return {
    periodo: range,
    visao_geral: {
      visitantes: vg.visitantes, sessoes: vg.sessoes, novos: vg.novos, recorrentes: vg.recorrentes,
      tempo_medio_s: vg.tempo_medio_s, rejeicao_pct: vg.rejeicao,
      dispositivos: vg.dispositivos, origens: vg.origens,
      top_paginas: vg.top_paginas, top_eventos: vg.top_eventos,
    },
    funil: fn.etapas.map(e => ({ etapa: e.label, usuarios: e.usuarios, abandono_pct: e.abandono_pct })),
    maior_abandono: fn.maior_abandono,
    secoes: sc.secoes,
    aquabook_buscas: sc.aquabook.buscas,
    aquabook_sem_resultado: sc.aquabook.sem_resultado,
    academy_videos: sc.academy,
    alertas: al.alertas,
  };
}
