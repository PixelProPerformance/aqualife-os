...
export function registrarLoja(deps) {
  ...
  app.post("/api/admin/loja/pedidos/:id/etiqueta", exigeLogin, exigeAdmin, async (req, res) => {
    try {
      ...
      const cartBody = {
        ...
        options: { insurance_value: +((ped.subtotal_cents || 0) / 100).toFixed(2), receipt: false, own_hand: false, non_commercial: true },
      };
  ...
  const emailNewsletter = (assunto, corpo, dest) => {
    const ola = dest && dest.nome ? `<p>Olá, ${escHtml(String(dest.nome).split(" ")[0])}!</p>` : "";
    return wrapEmail(assunto, ola + `<div>${nl2br(corpo)}</div>`,
      "Você recebeu este e-mail porque é cliente ou contato da Aqualife Aquarismo. Se não deseja mais receber, responda este e-mail pedindo o descadastro.");
  };
  ...
  app.post("/api/admin/comunicacoes/newsletter", exigeLogin, exigeAdmin, async (req, res) => {
    try {
      ...
      if (!lista.length) return res.status(400).json({ erro: "Nenhum destinatário encontrado para este público." });

      let enviados = 0, falhas = 0;
      for (let i = 0; i < lista.length; i++) {
        const d = lista[i];
        try {
          const r = await enviarEmail({ para: d.email, assunto, html: emailNewsletter(assunto, corpo, d) });
          if (r && r.ok === false) {
